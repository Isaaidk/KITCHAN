...
class UberWebhookUseCase:
    ...
    async def process_notification(self, payload: UberWebhookPayload) -> None:
        ...
        print(f"🛎️ [NEGOCIO] ¡Nueva orden detectada! ID Uber: {payload.meta.resource_id}")
        ...
        # ========================================================
        restaurante_id = await self.token_cache.get_restaurante_id_by_store(store_id_uber)
        ...
        print(f"✅ Mapeo exitoso: Tienda Uber {store_id_uber} pertenece al tenant {restaurante_id}")
        ...
        if not token:
            print(f"🚨 ERROR: No se encontró App Token para el restaurante {restaurante_id}")
            raise HTTPException(status_code=500, detail="Token no disponible para descargar la orden")

        print(f"📥 Descargando detalles de la orden {order_id_uber}...")
        
        orden_uber_detalles = await self.uber_api.get_order_details(order_id_uber, token)

        if not orden_uber_detalles:
            print(f"🚨 ERROR: No se pudo descargar la orden {order_id_uber} desde Uber")
            raise HTTPException(status_code=502, detail="Error descargando detalles de la orden")
        ...
        print(f"📦 DTO Construido: {orden_dto.nombre_cliente} - ${orden_dto.total} ({len(orden_dto.items)} items)")
